...
const EMAIL_WARN_KEY = "grunteo.emailWarned";
...
function remindEmailVerification(user) {
  if (!user || user.emailVerified) {
    try { localStorage.removeItem(EMAIL_WARN_KEY); } catch (_) {}
    return;
  }
  try {
    const warnedFor = localStorage.getItem(EMAIL_WARN_KEY);
    if (warnedFor === user.uid) return;
    localStorage.setItem(EMAIL_WARN_KEY, user.uid);
  } catch (error) {
    console.warn("[auth][warnFlag]", error);
  }
  notify("Twoje konto nie zostało jeszcze potwierdzone. Sprawdź skrzynkę e-mail, aby dokończyć rejestrację.", "warning");
}
